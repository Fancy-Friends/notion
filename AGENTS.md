...
npm run provider -- notion            # regenerate this repo
npm run provider -- notion --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/notion-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/notion-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/notion-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-notion` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/notion/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/notion` |
| npm | `@particle-academy/notion-ui`, `@particle-academy/notion-js` |
| Packagist | `particle-academy/notion-php` |
| PyPI | `fancy-notion` |
...
**Default to plain HTTP.** A Notion SDK is third-party code and is subject to
