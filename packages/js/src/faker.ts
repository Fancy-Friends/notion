 ...
 * NAMES Notion actually publishes, so an author can wire {{ $json.data.id }}
...
function fakePageMarkdownGet({ config, fake }: FakeRequest): unknown {
  return {
...
export const notionFaker: ConnectorFaker = (operation, request) => {
  switch (operation) {
    case "page_markdown_get":
      return fakePageMarkdownGet(request);
