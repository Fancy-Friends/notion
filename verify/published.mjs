/*
 * Notion — the published npm packages.
 *
...
import { notionFaker } from "@particle-academy/notion-js";
import { NOTION_KINDS } from "@particle-academy/notion-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/notion-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = notionFaker(operation, fakeRequest("notion", operation, config));
...
assert.equal(NOTION_KINDS.length, 1);
for (const kind of NOTION_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/notion-ui"}`);
